    ...
    <nav-view :scrollbar="true" :loading="loading" padding="p-5">
        ...
        <template slot="drawer">
            <ul class="v-menu">
                ...
                <li><router-link :to="{ name: 'jobList' }">Related Requests</router-link></li>
                <li><router-link :to="{ name: 'jobList' }">Related Contacts</router-link></li>              
            </ul>
        </template>
...
export default {
  ...
  mounted() {
    ...
    Promise.all([p1, p2])
      .then(() => {
        let job = this.getList("job", item => item.id == this.id)[0];
