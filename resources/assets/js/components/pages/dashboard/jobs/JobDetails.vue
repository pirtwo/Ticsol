    ...
    <nav-view :scrollbar="true" :loading="loading" padding="p-5">
        ...
        <template slot="drawer">
            <ul class="v-menu">
                ...
                <li>
                    <router-link :to="{ name: 'activityList', params : { col: 'job_id', opt: 'eq', val: this.id } }">Activity
                        Reports</router-link>
                </li>
                <li>
                    <router-link :class="[{'disabled' : this.relatedJobs === null}, 'btn btn-link' ]" role="button" :aria-disabled="this.relatedJobs === null" :to="{ name: 'jobList', params : { col: 'id', opt: 'in', val: this.relatedJobs } }">Related
                        Jobs</router-link>
                </li>
                <li>
                    <router-link :class="[{'disabled' : this.relatedRequests === null}, 'btn btn-link' ]" role="button" :aria-disabled="this.relatedRequests === null" :to="{ name: 'requestList', params : { col: 'job_id', opt: 'in', val: this.relatedRequests } }">Related
                        Requests</router-link>
                </li>
                <li>
                    <router-link :class="[{'disabled' : this.relatedContacts === null}, 'btn btn-link' ]" role="button" :aria-disabled="this.relatedContacts === null" :to="{ name: 'contactList', params : { col: 'id', opt: 'in', val: this.relatedContacts } }">Related
            ...
            <form class="needs-validation" novalidate>
                ...
                <div class="form-group">
                    <div class="form-row">
                        ...
                        <div class="col-sm-10">
                            <select-box v-model="form.parent_id" :data="jobs" :default="defaultParent" :multi-select="false"
                                id="parent_id" name="jobParent" placeholder="select parent..." search-placeholder="search..."></select-box>
                        </div>
                    </div>
                </div>
                ...
                <div class="form-group">
                    <div class="form-row">
                        ...
                        <div class="col-sm-10">
                            <select-box v-model="form.form_id" :data="profiles" :default="defaultProfile" :multi-select="false"
                                id="form_id" name="jobProfile" placeholder="select profile..." search-placeholder="search..."></select-box>
                        </div>
                    </div>
                </div>
...
export default {
  ...
  computed: {
    ...
    //     return this.currentJob.activities.reduce((a,c,i) => a + (i==0? '':',') + c.id, '');
    //   else return null;
    // },
  ...
  mounted() {
    ...
    let p1 = this.fetch({
      resource: "job",
      query: { with: "childs,contacts,requests" }
    });
    ...
    Promise.all([p1, p2])
      .then(() => {
        ...
        this.formData = Object.assign(
          {},
          this.formData,
          JSON.parse(this.currentJob.meta)
        );
